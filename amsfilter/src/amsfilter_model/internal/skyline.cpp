  ...
  const auto n_values = [&]() {
    ...
    for ($u64 n_log2 = n_lo_log2; n_log2 <= n_hi_log2; n_log2++) {
      const std::vector<$f64> exp{
          n_log2 + 0 * 0.0625,  n_log2 + 1 * 0.0625,  n_log2 + 2 * 0.0625,
          n_log2 + 3 * 0.0625,  n_log2 + 4 * 0.0625,  n_log2 + 5 * 0.0625,
          n_log2 + 6 * 0.0625,  n_log2 + 7 * 0.0625,  n_log2 + 8 * 0.0625,
          n_log2 + 9 * 0.0625,  n_log2 + 10 * 0.0625, n_log2 + 11 * 0.0625,
          n_log2 + 12 * 0.0625, n_log2 + 13 * 0.0625, n_log2 + 14 * 0.0625,
          n_log2 + 15 * 0.0625, n_log2 + 16 * 0.0625,
      };
      //      const std::vector<$f64> exp {
      //          n_log2 + 0 * 0.125,
      //          n_log2 + 1 * 0.125,
      ...
      for (auto e : exp) {
        u64 n = std::pow(2, e);
        if ((n * b_lo) > m_max)
          continue; // make sure to not exceed the max filter size
        n_s.insert(n);
      }
    }
...
auto get_tw_values() {
  const std::vector<$u64> tw_values = [&]() {
    ...
    for ($u64 tw_log2 = tw_lo_log2; tw_log2 <= tw_hi_log2; tw_log2++) {
      const std::vector<$f64> exp{
          tw_log2 + 0 * 0.03125,  tw_log2 + 1 * 0.03125,
          tw_log2 + 2 * 0.03125,  tw_log2 + 3 * 0.03125,
          tw_log2 + 4 * 0.03125,  tw_log2 + 5 * 0.03125,
          tw_log2 + 6 * 0.03125,  tw_log2 + 7 * 0.03125,
          tw_log2 + 8 * 0.03125,  tw_log2 + 9 * 0.03125,
          tw_log2 + 10 * 0.03125, tw_log2 + 11 * 0.03125,
          tw_log2 + 12 * 0.03125, tw_log2 + 13 * 0.03125,
          tw_log2 + 14 * 0.03125, tw_log2 + 15 * 0.03125,
          tw_log2 + 16 * 0.03125, tw_log2 + 17 * 0.03125,
          tw_log2 + 18 * 0.03125, tw_log2 + 19 * 0.03125,
          tw_log2 + 20 * 0.03125, tw_log2 + 21 * 0.03125,
          tw_log2 + 22 * 0.03125, tw_log2 + 23 * 0.03125,
          tw_log2 + 24 * 0.03125, tw_log2 + 25 * 0.03125,
          tw_log2 + 26 * 0.03125, tw_log2 + 27 * 0.03125,
          tw_log2 + 28 * 0.03125, tw_log2 + 29 * 0.03125,
          tw_log2 + 30 * 0.03125, tw_log2 + 31 * 0.03125,
          tw_log2 + 32 * 0.03125,
      };
      //      const std::vector<$f64> exp {
      //    tw_log2 +  0 * 0.0625,
  ...
  auto thread_fn = [&](u32 thread_id) {
    while (true) {
      ...
      for (std::size_t i = idx_begin; i < idx_end; ++i) {
        ...
        //      std::cout << "m=" << opt_m << ", o=" << opt_o << ", c=" << opt_c
        //      << std::endl;
      }
    }
  };
...
//===----------------------------------------------------------------------===//
SkylineEntry Skyline::lookup(const std::size_t n, f64 tw) {
